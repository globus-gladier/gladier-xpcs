...
def authorize():
    ...
    for tool in test.tools:
        required_inputs = required_inputs.union(set(getattr(tool, 'required_input', [])))
    inputs = test.get_input()
    user_inputs = {
        ri: input(f'Set value for "{ri}"> ') for ri in required_inputs if ri not in inputs['input']
    }
...
def run_test():
    ...
    authorizers = {scope: globus_sdk.RefreshTokenAuthorizer(data['refresh_token'], naac)
