...
def arg_parse():
    ...
    parser.add_argument(
        "--deployment",
        "-d",
        default="nick-polaris",
        help=f"Deployment configs. Available: {list(deployment_map.keys())}",
    )
    parser.add_argument(
        "--results",
        default="/data/xpcs8/2019-1/comm201901/ALCF_results/",
        help="Clutch transfer location for corr results on hdf file",
    )
...
if __name__ == "__main__":
    ...
    flow_input = {
        "input": {
            ...
            "funcx_endpoint_non_compute": depl_input["input"][
