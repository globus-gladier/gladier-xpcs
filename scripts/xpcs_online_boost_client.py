...
def arg_parse():
    ...
    parser.add_argument(
        "--hdf",
        help="Path to the hdf (metadata) file",
        default="/XPCSDATA/2019-1/comm201901/cluster_results/"
        "A001_Aerogel_1mm_att6_Lq0_001_0001-1000.hdf",
    )
    parser.add_argument(
        "--raw",
        help="Path to the raw data file. Multiple formats (.imm, .bin, etc) supported",
    ...
    parser.add_argument(
        "--atype",
        default="Both",
        help="Analysis type to be performed. Available: Multitau, Twotime",
    )
    parser.add_argument(
        "--gpu_flag",
        type=int,
    ...
    parser.add_argument(
        ...
        default="368beb47-c9c5-11e9-b455-0efb3ba9a670",
    )
    parser.add_argument(
        "--deployment",
        "-d",
        default="aps8idi-polaris",
        help=f"Deployment configs. Available: {list(deployment_map.keys())}",
    )
    parser.add_argument(
        "--batch_size", default="256", help="Size of gpu corr processing batch"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if not deployment:
        raise ValueError(
            f"Invalid Deployment, deployments available: {list(deployment_map.keys())}"
        )
    elif deployment.service_account and not (
        os.getenv("GLADIER_CLIENT_ID") and os.getenv("GLADIER_CLIENT_SECRET")
    ):
        raise ValueError(
            "Deployment requires setting GLADIER_CLIENT_ID and GLADIER_CLIENT_SECRET"
        )
    ...
    flow_run = corr_flow.run_flow(
        flow_input=flow_input, label=corr_run_label, tags=["aps", "xpcs"]
    )
