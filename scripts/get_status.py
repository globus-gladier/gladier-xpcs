...
from gladier_xpcs.flows import XPCSBoost, XPCSEigen
...
def callback(
    scopes: List[str],
) -> Mapping[str, Union[AccessTokenAuthorizer, AccessTokenAuthorizer]]:
...
def arg_parse():
    ...
    parser.add_argument(
        "--step", help="The inside the flow execution to check", default=None
    )
    parser.add_argument(
        "--interval", help="Interval between checking statuses", type=int, default=90
    )
    parser.add_argument(
        "--gpu",
        help="Whether the flow is the GPU flow rather than online flow.",
        action="store_true",
        default=False,
    )
...
def get_current_state_name(run_status):
    ...
    print(
        "BUG ENCOUNTERED! An unexpected status was returned by the flows "
        "service, a dump of the response is listed below. Please send this "
        "to us so we can fix it. Thanks!",
        file=sys.stderr,
    )
...
if __name__ == "__main__":
    ...
    if args.step not in flow_states:
        print(
            f'"{args.step}" is not valid, please choose from: {", ".join(flow_states)}'
        )
        sys.exit(-1)
