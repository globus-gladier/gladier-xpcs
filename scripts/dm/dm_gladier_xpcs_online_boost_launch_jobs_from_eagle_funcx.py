...
def glob_xpcs_datasets(path: str = None, regex: str = None, extensions: Tuple = None, **data) -> List[str]:
    """
    ...
    datasets = [str(p) for p in walk(path) if
                (not regex or p.match(regex)) and 
                (not extensions or p.suffix in extensions)]
    metadata_files = []
    for d in datasets:
        hdf_file = None
...
if __name__ == '__main__':
    args = parse_args()
    ct = CollectionTranslator('/eagle/XPCS-DATA-DYS/', alternate_path='/lus/eagle/projects/XPCS-DATA-DYS/')
    
    fx = FuncXExecutor(endpoint_id = '553e7b64-0480-473c-beef-be762ba979a9')
    future = fx.submit(glob_xpcs_datasets, ct.to_posix(args.input_dir), args.filter, ['.imm', '.bin', '.h5'])
